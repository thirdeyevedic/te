...
function notificationHtml(d: Record<string, string>): string {
  const row = (label: string, value?: string) =>
    value ? `<tr><td style="padding:6px 14px 6px 0;color:#8a8580;font-size:13px;vertical-align:top">${label}</td><td style="padding:6px 0;color:#171717;font-size:14px">${value}</td></tr>` : "";
  ...
  <h1 style="font-size:22px;font-weight:normal;margin:0 0 20px">New Enquiry — ${d.event_type || "General"}</h1>
  <table style="border-collapse:collapse;margin-bottom:20px">
    ${row("Name", d.name)}${row("Email", `<a href="mailto:${d.email}">${d.email}</a>`)}
    ${row("Phone / WhatsApp", d.phone)}${row("Destination", d.destination)}
    ${row("Guests", d.guests)}${row("Timeframe", d.timeframe)}
  </table>
  ${d.vision ? `<p style="color:#8a8580;font-size:12px;letter-spacing:.2em;margin:0 0 8px">VISION</p><p style="white-space:pre-wrap;color:#171717;font-size:15px;line-height:1.7;border-left:2px solid #B89B5E;padding-left:16px">${d.vision}</p>` : ""}
</div>`;
}
